...
function loginPrompt(program, account) {
  ...
  var result = prompts.promptUsernameAndPassword(null,
                                                 program.name + " Login",
                                                 "Enter your username and password for "+program.name+", this will be securely stored in the Password Manager.",
    ...
    var contentScriptFile = [addon.data.url('jquery-1.4.4.min.js'),
      ...
      tabs.open({
        ...
        onReady: function onOpen(tab) {
          ...
          tworker.port.on('loginFailure', function() {
            console.log("received loginFailure!");
            scanner.worker.port.emit('loginFailure', scanner.args);
          });
          tworker.port.on('data', function(data) {
            console.log("scanner got data "+JSON.stringify(data));
            data.balance = data.balance.replace(',','');
            data.statusMiles = data.statusMiles.replace(',','');
            try {
              data.expiration = new Date(Date.parse(data.expiration)).toLocaleDateString();
            } catch(e) {
              console.log("unable to parse date");
            }
            scanner.args.data = data;
            scanner.worker.port.emit("account-refresh", scanner.args);
            // we don't logout when doing this.
            //tworker.port.emit('signout');
            tworker.destroy();
          });
        }
      });
  ...
  fetchData: function() {
    ...
    let page = pageMod.PageMod({
      ...
      onAttach: function onAttach(worker) {
        ...
        worker.port.on('loginFailure', function() {
          console.log("received loginFailure!");
          scanner.worker.port.emit('loginFailure', scanner.args);
          page.destroy();
          thePage.destroy();
...
let AboutAwards = {
  ...
  newChannel: function(aURI) {
    ...
    let channel = ios.newChannel(addon.data.url("awards.html"), null, null);
